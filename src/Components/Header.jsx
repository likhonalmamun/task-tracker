...
import { makeDark, makeLight } from "../Redux/theme";
import {
  FaHome,
  FaPlus,
  FaCheck,
...
const Header = () => {
  ...
  const changeTheme = () => {
    console.log(dark);
    if (!dark) {
      console.log("trying to make dark");
      dispatch(makeDark());
      return;
    }
    if (dark) {
      console.log("trying to make light");
      dispatch(makeLight());
      return;
    }
  };
  return (
    <div className="flex px-5 lg:px-20 bg-gray-100 h-20 items-center  justify-between">
      ...
      <div>
        <ul className="lg:flex font-semibold text-lg hidden text-blue-500 items-center gap-5">
          ...
          <li>
            <label className="themeSwitcherThree relative inline-flex cursor-pointer select-none items-center">
              <input
                ...
                className="sr-only"
              />
              <span className="mr-[18px] text-sm  text-blue-500 font-bold">
                {dark ? "Dark" : "Light"}
              </span>
              <div
                htmlFor="themeSwitcherThree"
                onClick={changeTheme}
                className="shadow-card flex h-[46px] w-[82px] items-center justify-center rounded-md bg-white"
              >
                <span className="light bg-primary flex h-9 w-9 items-center justify-center rounded text-white">
                  <svg
                    width="16"
                    height="16"
                    viewBox="0 0 16 16"
                ...
                <span className="dark text-body-color flex h-9 w-9 items-center justify-center rounded">
                  <svg
                    width="16"
                    height="16"
                    viewBox="0 0 16 16"
        ...
        {/* here start the menu button and options */}

        <Menu as="div" className="relative inline-block  lg:hidden text-left">
          <div>
            <Menu.Button className="inline-flex w-full justify-center rounded-md bg-slate-200 px-4 py-2 text-lg font-bold  text-blue-500 hover:bg-opacity-30 focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75">
              <FaBars></FaBars>
            </Menu.Button>
          </div>
          <Transition
                  ...
                  {({ active }) => (
                    <Link to="/my-tasks">
                      <button
                        className={`${
                          active ? "bg-violet-500 text-white" : "text-gray-900"
                        } group gap-2 flex w-full items-center rounded-md px-2 py-2 text-sm`}
                      >
                        <FaTasks></FaTasks>
                        My Tasks
                      </button>
                    ...
                    >
                      <label className="themeSwitcherThree relative inline-flex cursor-pointer select-none items-center">
                        ...
                        <div
                          htmlFor="themeSwitcherThree"
                          onClick={changeTheme}
                          className="shadow-card flex h-[25px] w-[82px] items-center justify-center rounded-md bg-white"
                        >
                          <span className="light bg-primary flex h-9 w-9 items-center justify-center rounded text-white">
                            <svg
                              width="16"
        ...
        {/* here ends the menu and options */}
      </div>
    </div>
  );
};
